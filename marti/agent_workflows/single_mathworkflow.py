"""
Single-round RL workflow for math tasks (without MCTS)
This workflow performs a single generation-evaluation cycle for math problems.
"""
...
from marti.agent_workflows.third_party.mcts_utils.ab_mcts.llm_generation_interface import (
    GenerationRequest,
    GenerationResult,
)
from marti.agent_workflows.third_party.mcts_utils.ab_mcts.tasks.math.task import MathProblem
from marti.agent_workflows.third_party.mcts_utils.math_prompt import problem_prompt
...
async def workflow(
    ...
    **kwargs,
    ...
    """
    Single-round RL workflow for math tasks.
    
    This workflow:
    1. Takes a math problem prompt
    2. Uses the first agent to generate a solution
    3. Evaluates the solution using MathProblem
    ...
    Args:
        ...
        task: Task identifier (should be "math")
    ...
    # Validate task type
    if task != "math":
        logger.warning(f"Task type is '{task}', but this workflow is designed for 'math' tasks")
    ...
    # Parse label - for math tasks, label can be the ground truth answer
    ...
    # Ensure ground_truth is a list (as expected by MathProblem)
    ...
    # MathProblem expects: prompt, ground_truth, meta_data, indice
    indice = str(prompt_id) if prompt_id else "0"
    math_problem = MathProblem.load_data(
        prompt=prompt,
        ground_truth=ground_truth,
        meta_data=metadata,
        indice=indice,
    ...
    # Build the math prompt (add instruction suffix)
    math_prompt = problem_prompt(math_problem)
    
    # Apply chat template
    input_messages = [{"role": "user", "content": math_prompt}]
    formatted_prompt = apply_template_with_tokenizer(tokenizer, math_prompt)
    ...
    # For math tasks, generation should be the same as action_text
    ...
    # Set generation attribute for compatibility with MathProblem.evaluate_on_test
    # MathProblem uses llm_answer.generation, but GenerationResult has action_text
    # We add generation as an attribute that points to action_text
    gen_result.generation = action_text
    
    # Evaluate the generation using MathProblem
    ...
    eval_results = math_problem.generate_eval_results(llm_answer=gen_result, kind="transform")
