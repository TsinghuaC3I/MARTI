...
from marti.agent_workflows.third_party.mcts_utils.ab_mcts.data_types import Action, ARCProbData, Grid
from marti.agent_workflows.third_party.mcts_utils.ab_mcts.llm_generation_interface import GenerationResult
from marti.agent_workflows.third_party.mcts_utils.ab_mcts.eval_result import EvalResult, EvalResultWithAns
from marti.agent_workflows.third_party.mcts_utils.ab_mcts.tasks.base import Task
...
class CodeProblem(Task):
    ...
    @classmethod
    def load_data(cls, prompt, ground_truth, meta_data) -> "CodeProblem":
        problem = {
            'prompt': prompt,
            'label': ground_truth,
            'metadata': meta_data,
    ...
    def generate_eval_results(
        ...
        if kind == "transform":
            results = self.run_transform_on_demos(llm_answer.action_text)
        else:
    ...
    def evaluate_on_test(
        ...
        eval_results = self.run_transform_on_tests_and_check(llm_answer.action_text)
        score = (
            1.0
